...
public class FastCollinearPoints {
    ...
    public FastCollinearPoints(Point[] points) {
        ...
        for (Point first : points) {
            Arrays.sort(pointsCopy, first.slopeOrder());
            ...
            for (int i = 1; i < pointsCopy.length; i++) {
                slope = first.slopeTo(pointsCopy[i]);
                if (slope == previousSlope) {
                    slopePoints.add(pointsCopy[i]);
                } else {
                    if (slopePoints.size() >= 3) {
                        slopePoints.add(first);
    ...
    private void addSegmentIfNew(List<Point> slopePoints, double slope) {
        ...
        if (endPoints == null) {
            ...
            segments.add(new LineSegment(first, endPoint));
        } else {
